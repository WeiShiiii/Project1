...
void process_input()
{
    ...
    switch (event.type)
    {
      ...
      case SDL_KEYDOWN:
        switch (event.key.keysym.sym)
        {
          ...
          case SDLK_l: g_drops_movement.y = -2.0;
          default: break;
        }
      default:
        break;
  ...
  if (key_state[SDL_SCANCODE_A]) {g_drops_movement.x = -0.5;}
  else if (key_state[SDL_SCANCODE_D]) {g_drops_movement.x = 0.5;}
...
void update() {
    float currentTicks = SDL_GetTicks() / 1000.0f;
    float deltaTime = currentTicks - g_previous_ticks;
    g_previous_ticks = currentTicks;
    ...
    // Circular motion for the beaker
    float beakerX = radius * cos(glm::radians(angle));
    float beakerY = radius * sin(glm::radians(angle));
    g_beaker_matrix = glm::mat4(1.0f);
    g_beaker_matrix = glm::translate(g_beaker_matrix, glm::vec3(beakerX, beakerY, 0.0f));
    g_beaker_matrix = glm::scale(g_beaker_matrix, glm::vec3(3.0f, 3.0f, 1.0f));

    // Sinusoidal motion for the drops
    float dropsX = 4.0f * sin(glm::radians(angle * 2.0f));
    float dropsY = 2.0f * cos(glm::radians(angle * 2.0f));
    g_drops_matrix = glm::mat4(1.0f);
    g_drops_matrix = glm::translate(g_drops_matrix, glm::vec3(dropsX, dropsY, 0.0f));
    g_drops_matrix = glm::rotate(g_drops_matrix, glm::radians(angle), glm::vec3(0.0f, 0.0f, 1.0f));
    g_drops_matrix = glm::scale(g_drops_matrix, glm::vec3(1.0f, 1.0f, 1.0f));
}
